...
let content = fs.readFileSync('backend/routes/stations.js', 'utf8');
...
const new_route_start = `router.get('/:code/live', async (req, res) => {
...
const fallback_replacement = `            _category: category,
            _isStale: true,
...
const end_replacement = `    const filtered = finalTrains.filter(Boolean);
...
fs.writeFileSync('backend/routes/stations.js', content);
